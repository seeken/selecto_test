...
# - Payment (Base Layer)
#
# Usage: mix run customer_cone.exs
...
defmodule SelectoCone.ConeLive do
          ...
          <div style="max-height: 24rem; overflow-y: auto;">
            <%= for customer <- @customers do %>
              <div
                ...
                style={"padding: 0.75rem; margin-bottom: 0.5rem; border: 1px solid #E5E7EB; border-radius: 0.25rem; cursor: pointer; #{if @selected_customer && @selected_customer.id == customer.id, do: "background: #DBEAFE; border-color: #3B82F6;", else: "background: white;"}"}
              >
            ...
            <div style="max-height: 24rem; overflow-y: auto;">
              <%= for rental <- @rentals do %>
                <div
                  ...
                  phx-value-id={rental.id}
                  style={"padding: 0.75rem; margin-bottom: 0.5rem; border: 1px solid #E5E7EB; border-radius: 0.25rem; cursor: pointer; #{if @selected_rental && @selected_rental.id == rental.id, do: "background: #D1FAE5; border-color: #10B981;", else: "background: white;"}"}
                >
                  <div style="font-weight: 500;">
                    Rental #<%= rental.id %>
                  </div>
  ...
  def handle_event("select_rental", %{"id" => id}, socket) do
    ...
    customer_id = socket.assigns.selected_customer.id
  ...
  defp load_customer(socket, customer_id) do
    ...
    rentals = list_rentals(customer.id)
  ...
  defp load_rental(socket, rental_id) do
    ...
    payments = list_payments(rental.id)
...
📍 Visit: http://localhost:4092/selecto_cone
